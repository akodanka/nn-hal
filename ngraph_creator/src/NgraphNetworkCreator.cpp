...
NgraphNetworkCreator::NgraphNetworkCreator(const Model& model, const std::string& plugin) : mModel(model), mNgraphNodes(std::make_shared<NgraphNodes>(mModel.operands.size())), mOpFctryInst(plugin, mNgraphNodes)
{
...
bool NgraphNetworkCreator::initializeModel() {
    ...
    for (const auto& operation : mModel.operations) {
        ...
        mNgraphNodes->setOperationOutput(operation.outputs[0], //TODO: Handle multiple Outputs(eg.LSTM). Assumption here : each node has only 1 output.
